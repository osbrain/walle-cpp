...
void motor_L_front(uint16_t speed);  // 左轮前进
void motor_L_back(uint16_t speed);   // 左轮后退
void motor_L_stop(uint16_t stopMode);// 左轮停止
void motor_R_front(uint16_t speed);  // 右轮前进
void motor_R_back(uint16_t speed);   // 右轮后退
void motor_R_stop(uint16_t stopMode);// 右轮停止

void motor_forward(uint16_t speed);	 // 左右电机同速前进
void motor_backward(uint16_t speed); // 左右电机同速后退
