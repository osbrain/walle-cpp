...
void lcd_gpio_config(void)
{
	  ...
	  gpio_output_options_set(LCD_SCL_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, LCD_SCL_PIN);
		...
		rcu_periph_clock_enable(LCD_SDC_RCU);
	  gpio_mode_set(LCD_SDC_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, LCD_SDC_PIN);
	  gpio_output_options_set(LCD_SDC_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, LCD_SDC_PIN);
	  ...
	  gpio_output_options_set(LCD_BLK_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, LCD_BLK_PIN);
		...
		gpio_bit_set(GPIOB,GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7|GPIO_PIN_12);
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void LCD_ShowPicture4040(u16 x1,u16 y1,u16 x2,u16 y2,const u8 pic[])
{
		int i;
	  LCD_Address_Set(x1,y1,x2,y2);
		for(i=0;i<1600;i++)
	  { 	
			LCD_WR_DATA8(pic[i*2+1]);	 
			LCD_WR_DATA8(pic[i*2]);			
	  }			
}
...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void LCD_ShowPicture200200(u16 x1,u16 y1,u16 x2,u16 y2,const u8 pic[])
{
		int i;
	  LCD_Address_Set(x1,y1,x2,y2);
		for(i=0;i<40000;i++)
