/********************************************************************************
  ...
  *           蓝牙临时诊断指令41~47用于测试电机IO，51~56用于测试PCA9685舵机输出。
  ******************************************************************************
  * 注意事项:
*********************************************************************************/
...
// 舵机使用可见的点动幅度：上电回中位，每次命令推进30度，在安全范围内自动反向。
#define SERVO_HOME_ANGLE          90U
#define SERVO_SAFE_MIN_ANGLE      45U
#define SERVO_SAFE_MAX_ANGLE      135U
#define SERVO_EYE_STEP_ANGLE      30U
#define SERVO_HEAD_STEP_ANGLE     30U
#define SERVO_ARM_STEP_ANGLE      30U
...
#define SERVO_TEST_DELAY_MS       500U
...
enum {
    ...
    CMD_SERVO_TEST_CH0 = 51,      // 调试：舵机通道0大幅摆动。
    CMD_SERVO_TEST_CH1 = 52,      // 调试：舵机通道1大幅摆动。
    CMD_SERVO_TEST_CH2 = 53,      // 调试：舵机通道2大幅摆动。
    CMD_SERVO_TEST_CH3 = 54,      // 调试：舵机通道3大幅摆动。
    CMD_SERVO_TEST_CH4 = 55,      // 调试：舵机通道4大幅摆动。
    CMD_SERVO_TEST_ALL = 56       // 调试：舵机通道0~4同步大幅摆动。
};
...
static void servo_test_write_all(uint8_t angle)
{
    ...
    setAngle(SERVO_RIGHT_EYE_CH, angle);
    setAngle(SERVO_HEAD_CH, angle);
    setAngle(SERVO_LEFT_ARM_CH, angle);
    setAngle(SERVO_RIGHT_ARM_CH, angle);
}
...
static void servo_test_channel(uint8_t channel)
{
    ...
    delay_1ms(SERVO_TEST_DELAY_MS);
    setAngle(channel, SERVO_HOME_ANGLE);
}
...
static void servo_test_all(void)
{
    ...
    delay_1ms(SERVO_TEST_DELAY_MS);
    servo_test_write_all(SERVO_HOME_ANGLE);
}

static void handle_servo_test_command(uint8_t cmd)
{
    switch (cmd) {
        case CMD_SERVO_TEST_CH0:
            servo_test_channel(SERVO_LEFT_EYE_CH);
            break;
        case CMD_SERVO_TEST_CH1:
            servo_test_channel(SERVO_RIGHT_EYE_CH);
            break;
        case CMD_SERVO_TEST_CH2:
            servo_test_channel(SERVO_HEAD_CH);
            break;
        case CMD_SERVO_TEST_CH3:
            servo_test_channel(SERVO_LEFT_ARM_CH);
            break;
        case CMD_SERVO_TEST_CH4:
            servo_test_channel(SERVO_RIGHT_ARM_CH);
            break;
        case CMD_SERVO_TEST_ALL:
            servo_test_all();
            break;
        default:
            break;
